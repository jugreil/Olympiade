...
@app.route('/goldMedalls_per_NOC/<string:noc>')
def medals3(noc):
...
@app.route('/get_Athletes_by_NOC_and_Event/<string:noc>/<string:event>')
def get_atheles_by_NOC_And_Event(noc, event):
    res = engine.execute(
...
def avg_age():
    res = engine.execute(
        "SELECT NOC, Team, avg(Age) from athlete_events GROUP by NOC")
    res = [(row[0], row[1], row[2]) for row in res]
    return j.dumps(res)
